package typings.std.global

import typings.std.stdInts.`10`
import typings.std.stdInts.`11`
import typings.std.stdInts.`12`
import typings.std.stdInts.`14`
import typings.std.stdInts.`1`
import typings.std.stdInts.`2`
import typings.std.stdInts.`3`
import typings.std.stdInts.`4`
import typings.std.stdInts.`5`
import typings.std.stdInts.`6`
import typings.std.stdInts.`7`
import typings.std.stdInts.`8`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* This class was inferred from a value with a constructor. In rare cases (like HTMLElement in the DOM) it might not work as you expect. */
@JSGlobal("CSSPropertyRule")
@js.native
/* standard dom */
open class CSSPropertyRule ()
  extends StObject
     with typings.std.CSSPropertyRule {
  
  /* standard dom */
  /* CompleteClass */
  override val CHARSET_RULE: `2` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val COUNTER_STYLE_RULE: `11` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val FONT_FACE_RULE: `5` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val FONT_FEATURE_VALUES_RULE: `14` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val IMPORT_RULE: `3` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val KEYFRAMES_RULE: `7` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val KEYFRAME_RULE: `8` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val MEDIA_RULE: `4` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val NAMESPACE_RULE: `10` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val PAGE_RULE: `6` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val STYLE_RULE: `1` = js.native
  
  /* standard dom */
  /* CompleteClass */
  override val SUPPORTS_RULE: `12` = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSRule/cssText) */
  /* standard dom */
  /* CompleteClass */
  var cssText: java.lang.String = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSPropertyRule/inherits) */
  /* standard dom */
  /* CompleteClass */
  override val inherits: scala.Boolean = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSPropertyRule/initialValue) */
  /* standard dom */
  /* CompleteClass */
  override val initialValue: java.lang.String | Null = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSPropertyRule/name) */
  /* standard dom */
  /* CompleteClass */
  override val name: java.lang.String = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSRule/parentRule) */
  /* standard dom */
  /* CompleteClass */
  override val parentRule: org.scalajs.dom.CSSRule | Null = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSRule/parentStyleSheet) */
  /* standard dom */
  /* CompleteClass */
  override val parentStyleSheet: org.scalajs.dom.CSSStyleSheet | Null = js.native
  
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSPropertyRule/syntax) */
  /* standard dom */
  /* CompleteClass */
  override val syntax: java.lang.String = js.native
  
  /**
    * @deprecated
    *
    * [MDN Reference](https://developer.mozilla.org/docs/Web/API/CSSRule/type)
    */
  /* standard dom */
  /* CompleteClass */
  override val `type`: Double = js.native
}
